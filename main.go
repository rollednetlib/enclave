...
func main() {
	...
	tlsCert := ReadConf("tlsCert");
	tlsKey := ReadConf("tlsKey");
	publicBind := ReadConf("publicBind");

	if tlsCert != "NONE" && tlsKey != "NONE" {
		fmt.Println("Starting TLS Server\nKey: ", tlsKey, "\nCert: ", tlsCert);
		log.Fatal(http.ListenAndServeTLS(publicBind, tlsCert, tlsKey,  publicRouter));
	} else {
...
func SignInPost(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	if !checkCred(email, password) {
		fmt.Printf("FAILED!");
		http.ServeFile(w, r, "signinfailed.html");
	} else {
...
/*	email, _ := r.Cookie("email")
	...
	if !authPage() {
		http.ServeFile(w, r, "unauthorized.html");
